...
- **Fase:** Aula 6 concluída; melhorias da [docs/professor/revisao-plano-rag.md](docs/professor/revisao-plano-rag.md) implementadas em blocos (documentação, E2E, state).
...
4. [x] **docs/referencia/pipeline-indexacao.md** — uso do CLI, config, env, assunto dos arquivos.
...
- **Arquivos criados/alterados:** config/indexing.yaml, src/indexing/*, data/sample_*.csv|json, docs/referencia/pipeline-indexacao.md, tests/test_indexing.py, pyproject.toml (pypdf), state_rag.md.
...
- [x] **docs/referencia/pipeline-indexacao.md** — documentado backend em memory_policy.yaml.
...
- [x] **docs/referencia/arquitetura.md** — criado com arquitetura completa melhorada (visão geral, diagrama, fluxo RAG index/query, decisões de desenho, config, testes).
...
Regra: ao iniciar Bloco N (N>0), carregar **apenas** state_rag.md e docs/referencia/arquitetura.md; implementar apenas o escopo do Bloco N; ao fim, atualizar state_rag.
...
- **Entregas:** pytest verde; docs/referencia/pipeline-indexacao.md atualizado se necessário; state_rag com "Aula 6 concluída"; pendências futuras listadas.
- **Critério de conclusão:** pytest verde; documentação atualizada.
- **Status:** Concluído.
- **Arquivos alterados:** docs/referencia/pipeline-indexacao.md.
...
- docs/referencia/arquitetura.md criado; docs/referencia/pipeline-indexacao.md atualizado; state_rag.md com blocos e checklist.

---

## Melhorias pós-revisão do plano (Blocos A, B, C)

Implementação das ações sugeridas em [docs/professor/revisao-plano-rag.md](docs/professor/revisao-plano-rag.md).
...
- **Entregas:** docs/referencia/arquitetura.md — seção 4 (fórmula `max(0, 1 - distance)`), seção 5 (decisão where vazio, tabela backends, nota embedding); docstrings em LongTermMemoryGateway e search_customer_insights; state_rag com "Critério de conclusão" nos Blocos 1–3.
- **Status:** Concluído.
- **Arquivos alterados:** docs/referencia/arquitetura.md, src/memory_gateway.py, state_rag.md.
...
- **Status:** Concluído.
- **Arquivos alterados:** state_rag.md, docs/professor/revisao-plano-rag.md.
