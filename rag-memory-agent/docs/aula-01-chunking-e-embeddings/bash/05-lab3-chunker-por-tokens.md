# Lab 3 — Construa seu próprio Chunker `by_tokens` (versão bash)

> **Shell:** bash (Linux, macOS ou Git Bash no Windows). Versão PowerShell: [aqui](../powershell/05-lab3-chunker-por-tokens.md).
...
Primeiro, inspecione os chunks por tokens (sem mexer em config):

```bash
python scripts/chunk_inspect.py \
  --file data/lab/lab_emprestimo.txt \
  --strategy by_tokens --size 64 --overlap 8 \
  --show-chunks
```
...
| `rm -rf data/chroma` falha ("arquivo em uso") | Feche outros terminais/processos Python do grupo e tente de novo |
