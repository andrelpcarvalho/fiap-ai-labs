# Lab 2 — Chunking × Qualidade de Retrieval (versão PowerShell)

> **Shell:** PowerShell (Windows). Versão bash (Linux/macOS/Git Bash): [aqui](../bash/04-lab2-chunking-retrieval.md).
...
| Erro de lock/permissão em `data\chroma_lab2` | Outro processo Python segura o SQLite; feche os demais terminais do grupo e rode com `--clean` |
| Hit rates todos iguais/estranhos | Este script **não** usa `config/indexing.yaml` (as configs são internas); confira se está comparando as saídas certas |
