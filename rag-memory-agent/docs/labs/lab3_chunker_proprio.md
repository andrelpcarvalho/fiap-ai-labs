# Lab 3 — Construa seu próprio Chunker (`by_tokens`)
...
- Encoding: `cl100k_base` (mesmo família GPT/muitos embeddings)
...
```bash
python scripts/chunk_inspect.py --strategy by_tokens --size 64 --overlap 8 --compare
```

Indexe (coleção principal do lab):

```bash
# limpe chroma do lab de 1h
rm -rf data/chroma   # Windows: rmdir /s /q data\chroma

python -m src.indexing \
  --config config/indexing.yaml \
  --input data/lab \
  --output out/chunks_by_tokens.json \
  --push
```

## Passo 4 — Reavaliar com Lab 2 (10 min)

Adicione ao `DEFAULT_CONFIGS` em `scripts/eval_retrieval.py` (se ainda não estiver):
...
**Restaure** `indexing.yaml` para `recursive` / 512 / 64 ao final, para não quebrar o lab de 1h dos colegas.
